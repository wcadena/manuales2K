...
class UploadHandler
{
    ...
    protected function initialize() {
		...
		///echo $this->get_server_var('REQUEST_METHOD');
		if($this->get_server_var('REQUEST_METHOD')=="GET"){
	...
	/*
	*para guardar datos, conversion de tipos 
	*/
	...
	/*
	*para subir los archvos!!!1 y guarda en base de datos
	*/
	protected function subirarchivos_basedatos($name, $size, $type, $filename="",$file_deleteURL=""){
		error_reporting(E_ALL ^ E_DEPRECATED);
		require_once('../../Connections/manuales7sd.php');
		$insertSQL = sprintf("INSERT INTO archivos_".$sitio_manuales7sd." (nombre, filesize, filetype, 	log_forma,file_deleteURL) 
		VALUES (%s, %s, %s, %s, %s)", 
		$this->GetSQLValueString($name, "text"), 
		$this->GetSQLValueString($size, "text"), 
		$this->GetSQLValueString($type, "text"),
		$this->GetSQLValueString($filename, "text"),
		$this->GetSQLValueString($file_deleteURL, "text")
    );
	...
	protected function borrararchivos_basedatos($name){
		error_reporting(E_ALL ^ E_DEPRECATED);
		require_once('../../Connections/manuales7sd.php');
		$insertSQL = sprintf("DELETE FROM archivos_".$sitio_manuales7sd."  WHERE `nombre` = %s", 
		$this->GetSQLValueString($name, "text")
    );
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
		...
		$this->subirarchivos_basedatos($file->name, $file->size, $file->type,print_r($file,true),$file->delete_url );
        return $file;
    }
	...
	protected function get_file_name_param_delete() {
        return isset($_GET['delfile']) ? basename(stripslashes($_GET['delfile'])) : null; 
    }
    ...
    public function delete($print_response = true) {
        $file_name = $this->get_file_name_param_delete();
		$this->borrararchivos_basedatos($file_name);
        $file_path = $this->get_upload_path($file_name);
