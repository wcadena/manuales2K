...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1") && filter_var($_POST['username'], FILTER_VALIDATE_EMAIL)) {
  $updateSQL = sprintf("UPDATE users_biblioteca SET username=%s, password=%s, group_id=%s, modified=CURRENT_DATE, carpeta_master=%s WHERE id=%s",
...
mysql_select_db($database_manuales7sd, $manuales7sd);
$query_rs_user = sprintf("SELECT * FROM users_biblioteca WHERE id = %s", GetSQLValueString($colname_rs_user, "int"));
  ...
  <table align="center">
    ...
    <tr valign="baseline">
      ...
      <select name="group_id">
        <option value="ADMIN" <?php if (!(strcmp("ADMIN", htmlentities($row_rs_user['group_id'], ENT_COMPAT, 'utf-8')))) {echo "selected=\"selected\"";} ?>>ADMIN</option>
        <option value="USER" <?php if (!(strcmp("USER", htmlentities($row_rs_user['group_id'], ENT_COMPAT, 'utf-8')))) {echo "selected=\"selected\"";} ?>>USER</option>
      </select></td>
    </tr>
	<tr valign="baseline">
